...
func InitZapSugarDefault() {
	...
	file := filepath.Join(filepath.Dir(filename), "user_operation_log", "user_operation_log.log")
	_, err := os.OpenFile(file, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
...
func InitLog() {
	...
	file := filepath.Join(filepath.Dir(filename), "logs", "logs.log")
	_, err := os.OpenFile(file, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
		log.Printf("创建/打开日志文件失败：%v", err)
		return
